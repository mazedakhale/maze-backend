...
```bash
curl -X POST https://maze-backend-production.up.railway.app/auth/login \
...
```bash
curl -X POST https://maze-backend-production.up.railway.app/auth/signup \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET https://maze-backend-production.up.railway.app/auth/profile \
